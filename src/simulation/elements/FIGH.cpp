...
int Element_FIGH::update(UPDATE_FUNC_ARGS)
{
	...
	case 1:
		...
		{
			if(figh->rocketBoots || !(sim->eval_move(PT_FIGH, figh->legs[4]-10, figh->legs[5]+6, nullptr)
			     && sim->eval_move(PT_FIGH, figh->legs[4]-10, figh->legs[5]+3, nullptr)))
				figh->comm = 0x01;
			else
				figh->comm = 0x02;
			...
			else if (!sim->eval_move(PT_FIGH, figh->legs[4]-4, figh->legs[5]-1, nullptr)
			    || !sim->eval_move(PT_FIGH, figh->legs[12]-4, figh->legs[13]-1, nullptr)
			    || sim->eval_move(PT_FIGH, 2*figh->legs[4]-figh->legs[6], figh->legs[5]+5, nullptr))
				figh->comm = (int)figh->comm | 0x04;
		}
		else
		{ 
			if (figh->rocketBoots || !(sim->eval_move(PT_FIGH, figh->legs[12]+10, figh->legs[13]+6, nullptr)
			      && sim->eval_move(PT_FIGH, figh->legs[12]+10, figh->legs[13]+3, nullptr)))
				figh->comm = 0x02;
			else
				figh->comm = 0x01;
			...
			if (figh->rocketBoots)
			{
			    ...
			    || !sim->eval_move(PT_FIGH, figh->legs[4]+4, figh->legs[5]-1, nullptr)
			    || sim->eval_move(PT_FIGH, 2*figh->legs[12]-figh->legs[14], figh->legs[13]+5, nullptr))
				figh->comm = (int)figh->comm | 0x04;
		}
		break;
	default:
